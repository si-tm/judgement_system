...
public:
    ...
    template <class B, class E, NUPACK_IF(can_construct<T, B &&> && can_construct<T, E &&>)>
    constexpr View(B &&b, E &&e) : m_begin(fw<B>(b)), m_end(fw<E>(e)) {};

    template <class B, NUPACK_IF(can_construct<T, B &&>)>
    constexpr View(B &&b, std::nullptr_t) : m_begin(fw<B>(b)) {}

    template <class E, NUPACK_IF(can_construct<T, E &&>)>
    constexpr View(std::nullptr_t, E &&e) : m_end(fw<E>(e)) {}

    template <class V, NUPACK_IF(can_construct<T, decltype(begin_of(std::declval<V &&>()))>)>
    constexpr View(V &&v) : m_begin(begin_of(v)), m_end(end_of(v)) {}
...
struct ShortcutComparable : std::integral_constant<bool, std::is_pointer_v<T> && std::is_pointer_v<U>> {};
...
constexpr bool operator<(T const &t, U const &u) {
    if constexpr(ShortcutComparable<const_iterator_of<T>, const_iterator_of<U>>::value)
        if (begin_of(t) == begin_of(u) && end_of(t) == end_of(u)) return false;
...
struct view_t {
    ...
    template <class T>
    View<T> operator()(T const &b, T const &e) const {return View<T>(b, e);}
    ...
    template <class V>
    constexpr auto operator()(V const & v) const {return (*this)(begin_of(v), end_of(v));}
};
...
template <class V>
auto key_view(V &&v) {return indirect_view(fw<V>(v), first_of);}
/// Make a view of items of a container with std::pair-ish elements
template <class V>
auto item_view(V &&v) {return indirect_view(fw<V>(v), second_of);}
/// Make two views for the first and second halves of a container
template <class V>
auto bisect(V const &v) {
    return make_array(view(begin_of(v), midpoint(v)), view(midpoint(v), end_of(v)));
}
/// Make a view for the last n elements of a container
template <class V>
auto last_n(V const &v, size_type_of<V> i) {return view(end_of(v) - i, end_of(v));}

/******************************************************************************************/

template <class V>
auto moved_view(V &v) {return view(move_begin(v), move_end(v));}

/******************************************************************************************/

template <class V>
using ConstView = View<typename V::const_iterator>;

template <class V>
using MutableView = View<typename V::iterator>;
...
// index of first null character from a non-null iterator or pointer
template <class T>
auto null_pos(T t)  {while (*t) ++t; return t;}
...
template <class V>
auto data_view(V &&v) {return view(std::data(v), std::data(v) + std::size(v));}

/******************************************************************************************/

// For each subsequence of a given length
template <class V, class F>
void for_each_subsequence(V const &v, std::size_t n, F &&f) {
    auto b = std::begin(v);
    auto const end = std::end(v);
    if (std::distance(b, end) < n) return;
    auto e = std::next(b, n);
    for (; e != end; ++b, ++e) f(view(b, e));
    f(view(b, e));
}

// For each subsequence of any length
template <class V, class F>
void for_each_subsequence(V const &v, F &&f) {
    for (std::size_t n = 1; n < std::size(v); ++n) for_each_subsequence(v, n, f);
}
