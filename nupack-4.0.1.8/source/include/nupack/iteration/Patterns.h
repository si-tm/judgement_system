...
template <class F>
void for_each_noncrossing_partition(vec<std::size_t> &v, std::size_t const next, std::size_t const n, F &&f) {
    auto const l = std::size(v);
    if (l == n) {
        f(v, next);
    } else {
        v.emplace_back(next);
        for_each_noncrossing_partition(v, next+1, n, f);
        for (std::size_t i = 0; i != l; ++i) {
            if (!std::any_of(v.begin() + i+1, v.begin() + l, [y=v[i]](auto x) {return x <= y;})) {
                v.back() = v[i];
                for_each_noncrossing_partition(v, next, n, f);
            }
        }
        v.pop_back();
    }
...
template <class F>
void for_each_noncrossing_partition(std::size_t n, F &&f) {
    vec<std::size_t> index;
    index.reserve(n);
    for_each_noncrossing_partition(index, 0, n, f);
}
