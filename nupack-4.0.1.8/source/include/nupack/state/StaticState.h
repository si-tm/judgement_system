...
struct StaticState : public Base_, public Indexable<StaticState<Base_, Loop_>> {
    ...
    template <class Y=SequenceList, class ...Ts>
    ...
    bool check_structure(BasePairing, bool throw_error) const;

    /// Check that the structure is valid for the user-supplied energy model
    void check_structure(BasePairing p) const {check_structure(p, true);}
    bool is_valid(BasePairing p) const {return check_structure(p, false);}

    /// Calculate the energy w.r.t. to a user-supplied energy model
    template <class Model>
    auto calculate_energy(Model const &, bool include_join=true) const;
    /// Build all the loops
    template <class ...Ts> void build(Ts const &...);
    /// Add a loop
    template <class ...Ts> void emplace_loop(Ts&& ...ts) {loops.emplace_back(fw<Ts>(ts)...);}
...
void StaticState<W, O>::build(Ts const &...ts) {
    NUPACK_ASSERT(sys.data, "Strand system pointer should not be null");
    loops.reserve(pairs.n_pairs() + len(sys.strands()));
...
bool StaticState<W, O>::check_structure(BasePairing p, bool throw_error) const {
    bool ok = true;
    pairs.for_each_pair([&](iseq i, iseq j) {
        auto const strand1 = sys.data->strand_map[i], strand2 = sys.data->strand_map[j];
        if (!p(strand1 != strand2, next(sys.sequence(), i), next(sys.sequence(), j))) {
            ok = false;

            if (throw_error) {
                auto const dotparens = this->dp();
                auto const sequences = this->sequence();
                auto const index1 = i - sys.data->nicks[strand1] - 1, index2 = j - sys.data->nicks[strand2] - 1;
                Base const base1 = sys.sequence()[i], base2 = sys.sequence()[j];
...
auto StaticState<W, O>::calculate_energy(Model const &model, bool include_join) const {
    auto e = sum(loops, [&](auto const &o){return model.loop_energy(o.sequences(), o.nick());});
    if (include_join) e += (len(sys.strands()) - len(complexes)) * model.join_penalty();
    return e;
}
...
void for_exterior_loops_in_complex(State &&w, StateComplexes::Indices const &v, F &&f) {
