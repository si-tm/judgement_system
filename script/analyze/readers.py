...
def get_conf_top(target_dir):
    files = glob.glob(target_dir + "/*")

    # which seq of target_dir
    target_c = ""
    for str in target_dir.split("/"):
        if "seq" in str:
            target_c = str[-1]
    
    top_name = glob.glob(target_dir + "/*top")
    conf_name = glob.glob(target_dir + "/last_conf_seq" + target_c + "*")

    if len(top_name) != 1:
        print("could'not get topologyfile in " + target_dir.split("/")[-1])
        return
    elif len(conf_name) != 1:
        print("could'not get configfile in " + target_dir.split("/")[-1])
        return
    else:
        print("toplogy file is " + top_name[0].split("/")[-1])
        print("conf file is " + conf_name[0].split("/")[-1])

    return conf_name[0], top_name[0]

def test():
    conffile,topologyfile = get_conf_top("../results_KakenhiEvolveDNA/seqA/A3/test_a3_200000_1")
    my_reader = LorenzoReader(conffile,topologyfile)
    print(my_reader)
